...
class RelatoriosController extends Controller
{
    ...
    private function filtrarFluxoCaixa($data)
    {
        ...
        if ($data['dataInicialAno'] && $data['dataInicialMes']) {
            $dataInicial = (new Carbon())->setDate(
                (int)$data['dataInicialAno'],
                (int)$data['dataInicialMes'],
                1
            )->startOfMonth();
        }
        ...
        foreach ($contasReceberDetalhamento as $conta) {
            ...
            $detalhamentoContasReceber[str_slug(
                $conta->orcamento->cliente->categoria->categoria
            )]['total'] += $conta->valor_pago;
            $detalhamentoContasReceber[str_slug(
                $conta->orcamento->cliente->categoria->categoria
            )]['categoria'] = $conta->orcamento->cliente->categoria->categoria;
        }
        ...
        foreach ($contasPagarDetalhamento as $conta) {
            ...
            $detalhamentoContasPagar[str_slug(
    ...
    {
        dd($data);
        $query = OrdensServico::with('orcamento.cliente', 'empresa:id,razao_social', 'mtrs.certificados')
            ->select(['id', 'orcamento_id', 'empresa_id', 'data_coleta', 'status']);
