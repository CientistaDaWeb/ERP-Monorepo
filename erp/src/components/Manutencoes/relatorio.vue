            ...
            <div class="row q-col-gutter-md">
              ...
              <div class="col col-md-3 col-xs-12">
                <q-select
                  ...
                  data-vv-name="peca"
                  v-validate="'required'"
                  :error="errors.has('peca')"
                  :error-message="errors.first('peca')"
                />
              </div>
            </div>
...
export default {
  ...
  methods: {
    getData () {
      ...
      this.$store.dispatch('fornecedor/loadList', {})
      this.$store.dispatch('peca/loadList', {})
    },
    filtrar () {
      ...
      this.$store.dispatch('relatorios/relatorio', { relatorio: 'manutencao', data: data })
        .then(() => {
  ...
  computed: {
    fornecedor () {
      return _.orderBy(this.$store.state.fornecedor.list.map(
        data =>
          ({
    ...
    peca () {
      return _.orderBy(this.$store.state.peca.list.map(
        data =>
          ({
            label: data.peca,
            value: data.id
