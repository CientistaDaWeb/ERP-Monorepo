...
export default {
  name: 'ContasReceberDashboardVencendo',
  components: {
    QTable,
    QTr,
    QTd,
  ...
  computed: {
    list () {
      return _.orderBy(this.$store.state.contasReceber.listVencendo, 'data_vencimento', 'ASC')
    },
    loading: {
      get () {
  ...
  mounted () {
    this.loading = true
    this.$store.dispatch('contasReceber/loadListVencendo').then((data) => {
      this.loading = false
    })
  }
}
