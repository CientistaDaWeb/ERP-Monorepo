...
export default {
  ...
  computed: {
    list () {
      return _.orderBy(this.$store.state.projetos.listHidro, 'ultima_interacao.data', 'ASC')
    },
    loading: {
      get () {
  ...
  mounted () {
    this.loading = true
    this.$store.dispatch('projetos/loadListHidro').then((data) => {
      this.loading = false
    })
  }
}
