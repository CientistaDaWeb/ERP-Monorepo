              ...
              >
                ...
                <td>
                  <q-btn
                    ...
                    size="xs"
                    color="primary"
                    class="print-hide float-left q-mr-md self-center"
                    @click="$router.push({name:'orcamentos.editar', params: {id: d.orcamento.id }})"
                  />
                  <span class="self-center">{{ d.orcamento.id }}</span>
                </td>
                <td>{{ d.data_geracao | formatDate('DD/MM/YYYY') }}</td>
                <td>{{ tipos[d.tipo] }}</td>
                <td align="right">
                  {{ d.valor | currency }}
                </td>
...
export default {
  ...
  methods: {
    ...
    filtrar () {
      ...
      this.$store.dispatch('relatorios/relatorio', { relatorio: 'nota-fiscal', data: data })
        .then(() => {
    ...
    exportar () {
      ...
      data.data_inicial = date.formatDate(data.data_inicial, 'YYYY-MM-DD')
      data.data_final = date.formatDate(data.data_final, 'YYYY-MM-DD')
      this.$store.dispatch('relatorios/csv', { relatorio: 'nota-fiscal-csv', data: data })
        .then(() => {
          const url = `${process.env.DATA_URL}api/download/nota-fiscal/csv/${this.arqCsv}`
  ...
  computed: {
    ...
    tiposOptions () {
      const status = this.$store.state.notasFiscais.tiposOptions
      status.unshift({
        label: 'Todos',
        value: ''
      })
