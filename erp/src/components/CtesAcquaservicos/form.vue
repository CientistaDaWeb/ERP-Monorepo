...
export default {
  ...
  methods: {
    getData () {
      if (this.id) {
        this.$store.dispatch('ctesAcquaservicos/loadItem', this.id)
      } else {
        this.$store.commit('ctesAcquaservicos/setItem', {})
      }
      ...
      this.$validator.validate()
        .then(result => {
          if (!result) {
            ...
            if (this.action === 'edit') {
              this.$store.dispatch('ctesAcquaservicos/updateItem', { data: data, id: this.id })
            } else {
              this.$store.dispatch('ctesAcquaservicos/saveItem', data)
                .then(() => {
                  this.$router.push({
                    name: 'usuarios.editar',
                    params: { id: this.$store.state.ctesAcquaservicos.currentId }
                  })
                })
            }
  ...
  computed: {
    model () {
      let store = this.$store.state.ctesAcquaservicos.item
      if (store.data) {
