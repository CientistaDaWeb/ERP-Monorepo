      ...
      >
        <q-btn-group outline>
          <q-btn
            color="positive"
            @click="$router.push(
              {name:'ordens-servico.novo', params: {orcamento_id: orcamentoId}}
            )"
      ...
      >
        ...
        >
          ...
          >
            <q-btn-group flat>
              <q-btn
                @click="$router.push({name:'ordens-servico.editar', params: {id: props.row.id, orcamento_id: props.row.orcamento_id }})"
                icon="fa fa-edit"
                size="sm"
                color="primary"
                glossy
...
export default {
  ...
  name: 'OrcamentosOrdensServicoDatatable',
  data: () => ({
    selected: [],
    pagination: {
      sortBy: 'id',
