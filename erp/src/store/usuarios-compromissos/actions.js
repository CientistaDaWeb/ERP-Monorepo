...
export const loadListCompromissos = ({ commit, state }) => {
  const module = state.module
  const url = `${process.env.DATA_URL}api/${module.url}/compromissos`
  return axios
    .get(url)
    .then(({ data }) => {
      commit('setListCompromissos', data)
      return data
    })
    .catch(error => {
      Notify.create({
