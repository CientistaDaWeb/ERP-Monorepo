...
class CtesAcquaservicos_Model extends WS_Model {

    protected $_status, $_doc_tipo, $_campos;

    public function __construct() {
        $this->_db = new CtesAcquaservicos_Db();
        $this->_title = 'Gerenciador de Ctes Acquaservicos';
        $this->_singular = 'Cte Acquaservicos';
        $this->_plural = 'Ctes Acquaservicos';
    ...
    public function setBasicSearch() {
        $this->_basicSearch = $this->_db->select()
                ...
                ->from(array('c' => 'ctes_acquaservicos'), array('*'))
                ...
                ->joinLeft(array('cr' => 'contas_receber'), 'cr.cte_acquaservicos_id = c.id', array('total' => 'SUM(cr.valor-cr.valor_retido)', 'faturas' => 'COUNT(cr.id)'))
    ...
    public function buscaUltimoId() {
        $sql = $this->_db->select()
                ->from(array('c' => 'ctes_acquaservicos'), array('id'))
                ->order('id DESC')
                ->limit(1);
...
// 01 - cDV  - Dígito Verificador da Chave de Acesso
    ...
    public function adjustToView(array $data) {
        $data['boletos'] = '<a href="/erp/Boleto-Itau/cte-acquaservicos/' . base64_encode($data['id']) . '" class="buttonLink" target="_blank">Boletos</a>';
...
//$cte = $this->adjustToView($cte);
        ...
        $fatura = $ContasReceberModel->buscaSomaPorCteAcquaservicos($cte_id);
        $faturas = $ContasReceberModel->buscaPorCteAcquaservicos($cte_id);
...
//verificar tpMed, nos componentes do valor tem que aparecer Efluente e valor, situação tributária
    ...
    public function relatorio($data) {
        $consulta = $this->_db->select()
                ->setIntegrityCheck(false)
                ->from(array('cte' => 'ctes_acquaservicos'), array('*'))
                ->joinInner(array('c' => 'clientes'), 'c.id = cte.cliente_id', array('cliente' => 'razao_social'))
                ->joinLeft(array('cr' => 'contas_receber'), 'cr.cte_acquaservicos_id = cte.id', array('total' => 'SUM(cr.valor-cr.valor_retido)'))
                ->order('cte.data ASC')
                ->order('cte.id ASC')
                ->group('cte.id');
    ...
    public function getSumByPeriod($dataInicial, $dataFinal, $empresa_id = '') {
        $sql = $this->_db->select()
                ->setIntegrityCheck(false)
                ->from(array('c' => 'ctes_acquaservicos'), array(''))
                ->joinInner(array('cr' => 'contas_receber'), 'cr.cte_acquaservicos_id = c.id', array('faturamento' => 'SUM(cr.valor)'))
