...
class Erp_RelatorioCteAcqualifeController extends Erp_Controller_Action {

    public function init() {
        $this->model = new CtesAcqualife_Model();
        parent::init();
        $this->model->_title = 'Relatório de CT-e Acqualife';
    }
    ...
    public function zipaarquivosAction() {
        ...
        $arquivos = array();
        foreach ($ids AS $id):
            $CteModel = new CtesAcqualife_Model();
        ...
        if (!empty($arquivos)):
            $WSFile = new WS_File();
            $data = date('U');
            $WSFile->create_zip($arquivos, UPLOAD_PATH . 'cte/relatorio_contador_acqualife-'.$data.'.zip', true);
            echo '<a href="/storage/file/relatorio_contador_acqualife-'.$data.'.zip/cte/save" class="botao" target="_blank">Download do Arquivo</a>';
        else:
