...
class Erp_CteAcqualifeController extends Erp_Controller_Action {

    public function init() {
        $this->model = new CtesAcqualife_Model();
        $this->form = WS_Form_Generator::generateForm('Ctes', $this->model->getFormFields());
        require_once('NfephpAcqualife/libs/CTeNFePHP.class.php');
        $cte = new CTeNFePHP();
        parent::init();
    }
    ...
    public function formularioAction() {
        ...
        /*
          include('NfephpAcqualife/libs/ToolsNFePHP.class.php');
          $tools = new ToolsNFePHP();
          $tools->setEmpresa('Acqualife');
...
//            $nfe->setEmpresa('Acqualife');
            ...
            if ($nfe->errMsg == '200 OK'):
                ...
                $data = NULL;
                $data['cte_acqualife_id'] = null;
                $ContasReceberModel = new ContasReceber_Model();
                $where = $ContasReceberModel->_db->getAdapter()->quoteInto('cte_acqualife_id = ?', $cte_id);
                $ContasReceberModel->_db->update($data, $where);
    ...
    public function previewAction() {
        ...
        try {
            ...
            require_once('NfephpAcqualife/libs/DacteNFePHP.class.php');
    ...
    public function faturasAction() {
        ...
        $items = $ContasReceberModel->buscaPorCteAcqualife($cte_id);
...
//$this->alerta('error', 'Falha ao criar o PDF.');
    ...
    public function arquivosAction() {
        ...
        $ordenservico = $OrdensServicoModel->getByCteAcqualife($cte_id);
        ...
        $arquivo = 'uploads/cte/Faturas_Acqualife_' . $cte_id . '.pdf';
        $filename = realpath($arquivo);
        if (is_file($filename)) {
        ...
        if ($this->_request->isPost()):
            if ($this->_hasParam('destinatarios')):
                if ($form->isValid($this->_request->getPost())) :
                    ...
                    $OrdensServicoModel = new OrdensServico_Model();
                    $selecionados = $OrdensServicoModel->buscarPorCteAcqualife($cte_id);
                    ...
                    try {
                        ...
                        $arquivo = 'uploads/cte/Faturas_Acqualife_' . $cte['id'] . '.pdf';
                        $filename = realpath($arquivo);
                        if (is_file($filename)) :
                            $at = new Zend_Mime_Part(file_get_contents($arquivo));
                            $at->type = 'application/pdf';
                            $at->disposition = Zend_Mime::DISPOSITION_ATTACHMENT;
                            $at->encoding = Zend_Mime::ENCODING_BASE64;
                            $at->filename = 'Faturas_Acqualife_' . $cte['id'] . '.pdf';
                            $mail->addAttachment($at);
                        endif;
