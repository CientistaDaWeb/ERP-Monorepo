...
class Ctes_Model extends WS_Model {

    protected $_status, $_doc_tipo, $_campos;

    public function __construct() {
        $this->_db = new Ctes_Db();
        $this->_title = 'Gerenciador de Ctes';
        $this->_singular = 'Cte';
        $this->_plural = 'Ctes';
    ...
    public function setBasicSearch() {
        $this->_basicSearch = $this->_db->select()
                ...
                ->joinLeft(array('cr' => 'contas_receber'), 'cr.cte_id = c.id', array('total' => 'SUM(cr.valor-cr.valor_retido)', 'faturas' => 'COUNT(cr.id)'))
    ...
    public function adjustToDb(array $data) {
        if (empty($data['codigo'])):
            ...
            $data['chave'] = $chave;
            $id = $cte['id'] + 1;
            $data['codigo'] = $this->calcula_chave_acesso('43', date('ym'), '09466930000100', '57', '001', substr('00000000' . $id, -9), '1', $chave);
        endif;
...
//$cte = $this->adjustToView($cte);
        ...
        $empresa = $EmpresasModel->buscaParaCte(1);
        ...
        $fatura = $ContasReceberModel->buscaSomaPorCte($cte_id);
        $faturas = $ContasReceberModel->buscaPorCte($cte_id);
...
//verificar tpMed, nos componentes do valor tem que aparecer Efluente e valor, situação tributária
    ...
    public function relatorio($data) {
        $consulta = $this->_db->select()
                ->setIntegrityCheck(false)
                ->from(array('cte' => 'ctes'), array('*'))
                ->joinInner(array('c' => 'clientes'), 'c.id = cte.cliente_id', array('cliente' => 'razao_social'))
                ->joinLeft(array('cr' => 'contas_receber'), 'cr.cte_id = cte.id', array('total' => 'SUM(cr.valor-cr.valor_retido)'))
                ->order('cte.data ASC')
                ->order('cte.id ASC')
                ->group('cte.id');
    ...
    public function getSumByPeriod($dataInicial, $dataFinal, $empresa_id = '') {
        $sql = $this->_db->select()
                ->setIntegrityCheck(false)
                ->from(array('c' => 'ctes'), array(''))
                ->joinInner(array('cr' => 'contas_receber'), 'cr.cte_id = c.id', array('faturamento' => 'SUM(cr.valor)'))
