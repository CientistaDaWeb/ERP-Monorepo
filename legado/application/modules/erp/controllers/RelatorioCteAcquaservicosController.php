...
class Erp_RelatorioCteAcquaservicosController extends Erp_Controller_Action {

    public function init() {
        $this->model = new CtesAcquaservicos_Model();
        parent::init();
        $this->model->_title = 'Relatório de CT-e Acquaserviços';
    }
    ...
    public function indexAction() {
        ...
        if ($this->_request->isPost()):
            ...
            if ($data['gera_csv']):
                ...
                $WF = new WS_File();
                $file = 'cte_acquaservicos-'.date('U').'.csv';
    ...
    public function zipaarquivosAction() {
        ...
        $arquivos = array();
        foreach ($ids AS $id):
            $CteModel = new CtesAcquaservicos_Model();
        ...
        if (!empty($arquivos)):
            $WSFile = new WS_File();
            $data = date('U');
            $WSFile->create_zip($arquivos, UPLOAD_PATH . 'cte/relatorio_contador_acquaservicos-'.$data.'.zip', true);
            echo '<a href="/storage/file/relatorio_contador_acquaservicos-'.$data.'.zip/cte/save" class="botao" target="_blank">Download do Arquivo</a>';
        else:
